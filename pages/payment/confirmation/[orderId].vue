          ...
          <div class="space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <div>{{ formatDate(ticket?.race?.start_time) }}</div>
              </div>
              <div>
                <div class="text-muted-foreground">Distance</div>
                <div>{{ ticket?.race?.distance_km }} km</div>
              </div>
              <div>
              ...
              <div>
                ...
                <div class="font-medium">{{ formatCurrency(ticket?.total_price_cents / 100, ticket?.race?.currency) }}</div>
              </div>
            </div>
...
const fetchTicket = async () => {
  ...
  try {
    ...
    if (response.tickets.length > 0) {
      ...
      error.value = response.message || 'Payment is being processed. Please wait...'
      
      // Auto-refresh after 3 seconds to check if tickets are created
      setTimeout(() => {
    ...
    error.value = err.message || 'Failed to load order information'
    paymentStatus.value = 'failed'
  } finally {
    loading.value = false
  }
